...
class YOLOX(SingleStageDetector):
    ...
    #     if self.freeze_all:
    #         super(YOLOX, self).train(False)
    #         for param in self.parameters():
    #             param.requires_grad = False
    #     else:
    #         super(YOLOX, self).train(mode)
...
@DETECTORS.register_module()
class YoloxSet(YOLOX):
    def __init__(self, orig_head, **kwargs):
        super(YoloxSet, self).__init__(**kwargs)
        orig_head.update(train_cfg=kwargs["train_cfg"])
        orig_head.update(test_cfg=kwargs["test_cfg"])
        self.orig_head = build_head(orig_head)

    def train(self, mode=True):
        if not mode:
            # self.bbox_head = self.orig_head
            self.bbox_head.orig_head = self.orig_head
            # print("It's USED!!!")
        else:
            self.bbox_head.orig_head = self.orig_head
        super(YOLOX, self).train(mode)
...
class YoloxSetPair(YOLOX):
    def forward_train(self,
        ...
        # Multi-scale training
        ref_img, ref_gt_bboxes = self._preprocess(ref_img, ref_gt_bboxes)

        super(SingleStageDetector, self).forward_train(ref_img, ref_img_metas)
        x = self.extract_feat(ref_img)
        losses = self.bbox_head.forward_train(x, ref_img_metas, ref_gt_bboxes, ref_gt_labels, None)
        # random resizing
        if (self._progress_in_iter + 1) % self._random_size_interval == 0:
            self._input_size = self._random_resize()
        self._progress_in_iter += 1

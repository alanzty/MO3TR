...
def parse_args():
    ...
    parser.add_argument('--config', default="../cfg/cfg_mo3tr_nf.py")
    # parser.add_argument('--checkpoint', default="/storage/alan/workspace/mmStorage/mot/cfg_mo3tr_2f_fpdb0.5_separate_fr2/epoch_100.pth")
    # parser.add_argument('--checkpoint', default="/storage/alan/workspace/mmStorage/mot/mo3tr_nf_fr2/epoch_120.pth")
    # parser.add_argument('--checkpoint', default="/storage/alan/workspace/mmStorage/mot/mo3tr_nf_fr2/epoch_120.pth")
    parser.add_argument('--checkpoint', default="/storage/alan/workspace/mmStorage/mot/mo3tr_temphs_fr5/epoch_20.pth")
    parser.add_argument('--show-dir', default="")
    parser.add_argument('--prop_thr', default=0.5, type=float)

    parser.add_argument('--out', help='output result file')
    parser.add_argument('--work-dir', default="/storage/alan/workspace/mmStorage/mot/mo3tr_temphs_fr5")
    parser.add_argument(
        '--fuse-conv-bn',
        action='store_true',
...
def main():
    ...
    if args.prop_thr is not None:
        cfg.model.tracker.init_track_thr = args.prop_thr
    ...
    pytorch_total_params = sum(p.numel() for p in model.temporal_model.parameters())
    ...
    if rank == 0:
        ...
        if args.eval:
            ...
            metric, str_summary = dataset.evaluate(outputs, **eval_kwargs)
            with open(osp.join(args.work_dir, "thr_tune.txt"), "a") as f:
                f.write(f"prop threshold is: {args.prop_thr} \n")
                f.write(str_summary)
                f.write("\n")
            print(metric)
            metric_dict = dict(

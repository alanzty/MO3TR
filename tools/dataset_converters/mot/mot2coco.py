...
def parse_args():
    ...
    parser.add_argument('-i', '--input', default="/storage/alan/ByteTrackDataset/MOT17/")
    parser.add_argument('-o', '--output', default="/storage/alan/ByteTrackDataset/MOT17/annotations")
    parser.add_argument(
        '--convert-det',
        action='store_true',
...
def main():
    ...
    for subset in sets:
        ...
        if args.SDP:
            out_file = osp.join(args.output, f'{subset + "-SDP"}_cocoformat.json')
        else:
        ...
        video_names = [vn for vn in video_names if "SDP" in vn]
